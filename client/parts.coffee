...
Template.reference_other_children.helpers
    # selector: -> Docs.findOne Session.get('editing_id')

    available_children: ->
        # Docs.find parent_id:Session.get('editing_id')
...
Template.doc_match.helpers
    match_doc: -> Docs.findOne Session.get('editing_id')
...
#         Meteor.call 'calculate_doc_completion', Session.get('editing_id')
...
Template.dev_footer.events
    ...
    # 'click #trickle_down': ->
    #     doc = Docs.findOne Session.get('editing_id')
    #     Meteor.call 'calculate_child_ancestor_array', Session.get('editing_id')
        
        
    'click #create_parent': ->
        parent_doc_id = Docs.insert {}
        Docs.update Session.get('editing_id'),
        ...
        if GoogleMaps.loaded()
            ...
            $("#google_places_field").geocomplete().bind("geocode:result", (event, result)->
                ...
                if confirm "change location to #{result.formatted_address}?"
                    Meteor.call 'update_location', Session.get('editing_id'), result
                )
    )
...
# Template.google_places_input.events
    ...
    #     console.log $('#google_places_field').geocomplete();
        ...
        # console.dir result
        # Meteor.call 'update_location', Session.get('editing_id'), result, (err,res)->
            # console./log res
