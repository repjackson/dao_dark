if Meteor.isClient
    Template.tribe_delta.onCreated ->
    ...
    Template.tribe_type_edit.onCreated ->
    ...
    Template.tribe_type_view.onCreated ->
        @autorun -> Meteor.subscribe 'schema_from_doc_id', Router.current().params._id
        @autorun -> Meteor.subscribe 'bricks_from_doc_id', Router.current().params._id
        @autorun -> Meteor.subscribe 'doc', Router.current().params._id, Router.current().params.type
        @autorun -> Meteor.subscribe 'type', 'field_template'

    Template.tribe_type_edit.events
        'blur .body': (e,t)->
            body = t.$('.body').val()
    ...
    Template.tribe_facet.onRendered ->
        Meteor.setTimeout ->
            $('.accordion').accordion()
        , 1500
    
    Template.tribe_facet.events
